...
(function () {
  ...
  const fresh = () => ({ day: 0, held: [], piles: Object.fromEntries(D.clans.map(c => [c.resource, 0])), mana: 20, cool: {}, raidsBroken: 0, raidsTaken: 0, sold: 0, seed: D.seed, grant: false, log: [], saved: Date.now() });
  ...
  function step(dt, quiet) { const d0 = Math.floor(S.day); S.day += dt; const d1 = Math.floor(S.day); if (d1 === d0) return; const T = turn(); const raidMul = T.id === 'turn-noon' ? 1.3 : T.id === 'turn-night' ? 0.7 : 1;
  ...
  const stats = [['heze', 'HEZE'], ['turn', 'the planet'], ['assets', 'the portfolio'], ['raids', 'raids'], ['bronze', 'bronze sold'], ['mana', 'the reserve']];
  ...
  function buildClans() { const box = $('#clans'); box.innerHTML = ''; for (const c of D.clans) { const sec = el('div', { class: 'clan' }, el('h3', {}, `${c.name} · ${c.house}`, el('small', {}, `${c.resource} every ${c.period} days · next on day ${Math.ceil((Math.floor(S.day) + 1) / c.period) * c.period}`)), el('p', {}, c.saga.slice(0, 160) + (c.saga.length > 160 ? '…' : '')));
  ...
  function cast(sp) { if (S.mana < sp.cost || (S.cool[sp.id] || 0) > 0) return; S.mana -= sp.cost; S.cool[sp.id] = sp.cooldown;
    ...
    save(); render(); }
  function render() { const T = turn(); setStat('heze', fmt(Dk.heze), 'the shared docket'); setStat('turn', T.name, T.does); setStat('assets', S.held.length, `${held('work').length} works · ${held('band').length} bands · ${held('pile').length} piles`); setStat('raids', `${S.raidsBroken} / ${S.raidsTaken}`, 'broken at the ford / taken'); setStat('bronze', fmt(S.sold), 'silicate and hydrogen, cast'); setStat('mana', `${S.mana} / 20`, 'one a day');
