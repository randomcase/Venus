...
(function () {
  ...
  const farm = () => read(FKEY, { harvested: 0, season: 0 });
  const fresh = () => ({ day: 0, built: [], grain: 0, drawn: 0, households: 0, fed: 0, prosperity: 1, festival: null, lastSeason: -1, sayingAt: 0, log: [], saved: Date.now() });
  ...
  function render() { const F = farm();
    setStat('heze', fmt(Dk.heze), 'the shared docket'); setStat('grain', fmt(S.grain), `${fmt(F.harvested || 0)} harvested at the farm in all`); setStat('households', S.households, `${S.fed} fed · ${built('halls').reduce((a, h) => a + h.houses, 0)} housed · ${built('wells').reduce((a, w) => a + w.waters, 0)} watered`);
    ...
    $('#log').innerHTML = S.log.map(l => `<div><b>d${l.t}</b>${l.m}</div>`).join('') || '<div>Nothing yet. A trade first; it needs the farm\'s harvest.</div>'; $('#clock').textContent = `${S.built.length} buildings · ${built('trades').length} trades`; }
