...
(function () {
  ...
  /* the rules: embedded from templates-rules/market.json at build, and overridden by a rulebook kept in this browser through the editor in the quarter */
  const RULES = (() => { let o = {}; try { o = (JSON.parse(localStorage.getItem('custom.v1') || 'null') || {})['templates-rules/market.json'] || {}; } catch (e) {} return Object.assign({}, D.rules, o.rules || o); })();
  ...
  const CAP = RULES.cap, DKEY = 'descent.v1', KEY = 'market.v1';
  ...
  let S = read(KEY, { day: 0, degree: RULES.defaultDegree, seed: D.seed, held: [], grant: false, log: [], paid: 0, saved: Date.now() });
  ...
  if (!S.grant) { S.grant = true; credit(RULES.grant, 'a seat on the market, against the cap'); note('Four farms, syndicated. Slide the degree and watch the board stop being tickers and start being weather.'); }
  const away = Math.min((Date.now() - S.saved) / 1000, RULES.awayHours * 3600); if (away > 5) S.day += away;
  ...
  function weight(fi, path) { let w = 1, k = fi + 1; for (let d = 1; d <= path.length; d++) { k = k * 4 + path[d - 1] + 1; w *= RULES.weightMin + RULES.weightSpan * (h32(S.seed, k, d) % 1000) / 1000; } return w; }
  function drift(fi, path, day) { const f = F[fi], key = keyOf(fi, path), d = path.length, P = f.period * RULES.driftPeriodMul, t = day / P, i = Math.floor(t), a = u(S.seed + 7, key, i) - 0.5, b = u(S.seed + 7, key, i + 1) - 0.5; return Math.exp((a + (b - a) * smooth(t - i)) * RULES.volatilityBase * (1 + d * RULES.volatilityPerDegree)); }
  ...
  const REGIMES = D.regimes.map(r => `<b>${r.name}</b> ${r.text}`);
  ...
  function step(dt) { const d0 = Math.floor(S.day); S.day += dt; const d1 = Math.floor(S.day); if (d1 === d0) return; for (const h of S.held) { const f = F[h.fi]; if (d1 % f.period === 0) { const div = h.qty * f.price * RULES.dividend / 4 ** h.path.length; credit(div, `dividend, ${h.id}`); S.paid += div; } } }
  ...
  function render() { const val = S.held.reduce((a, h) => a + h.qty * price(h.fi, h.path, S.day), 0), paid = S.held.reduce((a, h) => a + h.paid, 0);
    ...
    const d = Math.min(S.degree, 4), n = 1 << d; $('#tape').innerHTML = Array.from({ length: RULES.tape }, (_, i) => { const k = h32(Math.floor(S.day / 5), i, 3), fi = k % 4, x = (k >> 2) % n, y = (k >> 12) % n, p = pathOf(x, y, d), now = price(fi, p, S.day), was = price(fi, p, S.day - 1), ch = (now - was) / was * 100; return `<span><b>${idOf(fi, p)}</b> ${money(now)} <i class="${ch >= 0 ? 'up' : 'dn'}">${ch >= 0 ? '+' : ''}${ch.toFixed(2)}%</i></span>`; }).join(''); }
  ...
  regime(); draw(); render(); setInterval(() => { step(1); if (S.degree < 8 || Math.floor(S.day) % 4 === 0) draw(); render(); }, RULES.secondsPerDay * 1000); setInterval(save, 5000); addEventListener('beforeunload', save);
})();
