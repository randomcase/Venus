...
<main>
  ...
  <div>
    ...
    <section><h2>The editor<i>any template on the ship, loaded, changed, kept in this browser, and handed back as a file</i></h2>
      <div class="row" style="display:flex;gap:6px;flex-wrap:wrap"><input id="ed-path" type="text" value="templates-quarter/library-table.json" style="flex:1;min-width:200px;font:inherit;color:var(--ink);background:var(--panel2);border:1px solid var(--edge);border-radius:9px;padding:6px 8px"><button id="ed-load">Load</button></div>
      ...
      <p style="color:var(--dim);font-size:12px;margin:8px 0 0" id="ed-note">Customizations live in this browser under custom.v1, keyed by path. The quarter reads its own back: change a room or a piece here and it changes on the plan. The download is the file to write into the yard, and the generators do the rest.</p>
    </section>
  </div>
</main>
