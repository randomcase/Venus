...
export const RULES = {
  ...
  coven: { ...DOCKET, intervalDays: 183, quorum: 2, of: 3, proposalEvery: 3, signChance: 12, deskLimit: 40, staleAfter: 183, activityEvery: 5, stealCooldown: 30, presentDays: 5, interferenceEvery: 6, interferenceChance: 10, suspicionDays: 14, relationEvery: 4, auditChance: 30, lendAmount: 6, rivalChance: 25 },
};
export const NOTES = {
  ...
  coven: { ...NOTES_DOCKET, intervalDays: 'days between syndications; six months, the interplanetary interval', quorum: 'signatures an event needs', of: 'custodians in a syndicate', proposalEvery: 'days between proposals arriving at the doors', signChance: 'chance in a hundred that a custodian signs on a given day, when they are signing on their own', deskLimit: 'proposals the desk holds before the oldest fall off', staleAfter: 'days after which an unsigned proposal counts against the bank\'s health', activityEvery: 'days between a syndicate rolling a new activity — quiet, planning, discovery or staged', stealCooldown: 'days a syndicate stays alert to a failed steal before it can be tried again', presentDays: 'days a syndicate shows green after carrying, for having presented', interferenceEvery: 'days between a president or king getting a chance to cast suspicion on a syndicate', interferenceChance: 'chance in a hundred that the interference check actually casts one', suspicionDays: 'days a suspicion cast by interference holds before it lifts on its own', relationEvery: 'days between a syndicate acting on its neighbor — audit, lend or rival, one roll each', auditChance: 'chance in a hundred a wizard audits the neighbor rather than skipping it', lendAmount: 'wax a full croft lends a neighbor running low, if it lends at all', rivalChance: 'chance in a hundred that two neighbors both staged compete rather than both carry clean' },
