...
(function () {
  ...
  const village = () => read(VKEY, { households: 0, prosperity: 1, festival: null });
  const fresh = () => ({ day: 0, built: [], citizens: 0, prosperity: 1, chartered: false, earned: 0, log: [], saved: Date.now() });
  ...
  function step(dt, quiet) { S.day += dt; const V = village();
    const housed = built('wards').reduce((a, w) => a + w.houses, 0), watered = built('aqueducts').reduce((a, w) => a + w.waters, 0), fed = built('stalls').reduce((a, s) => a + s.feeds, 0) + built('guilds').length * 3;
    ...
    let paid = built('guilds').reduce((a, g) => a + g.dues, 0) * staffed * mul * keep * S.prosperity * dt + built('tenements').reduce((a, t) => a + t.rent, 0) * dt;
    ...
    if (paid > 0) { credit(paid, 'dues and rents'); S.earned += paid; }
  ...
  function render() { const V = village();
    setStat('heze', fmt(Dk.heze), 'the shared docket'); setStat('citizens', S.citizens, `${V.households} from the village · ${built('wards').reduce((a, w) => a + w.houses, 0)} housed here · ${built('aqueducts').reduce((a, w) => a + w.waters, 0)} watered`); setStat('prosperity', `×${S.prosperity}`, V.festival ? V.festival.name.replace('festival', 'fair') : 'no fair');
    setStat('charter', S.chartered ? 'chartered' : `${built('guilds').length}/5 guilds`, S.chartered ? 'the town is its own' : 'five guilds and fifteen citizens'); setStat('day', `day ${Math.floor(S.day)}`, `${fmt(S.earned)} HEZE in dues and rents`);
    ...
    $('#log').innerHTML = S.log.map(l => `<div><b>d${l.t}</b>${l.m}</div>`).join('') || '<div>Nothing yet. A guild first; it needs citizens, and citizens come from the village.</div>'; $('#clock').textContent = `${S.built.length} buildings · ${built('guilds').length} guilds`; }
